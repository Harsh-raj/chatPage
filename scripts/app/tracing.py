"""
Langfuse tracing helpers for the ingestion service.
...
headers, so this service's ingestion span nests under the gateway's span in the
...
def downstream_headers(span) -> dict:
    """Headers to attach to this service's own outgoing calls (to generation
    for page/document summaries, to retrieval for indexing) so those spans
    nest under this service's current span instead of each becoming its own
    disconnected trace. Takes the span object's .trace_id/.id directly, same
    reasoning as gateway/app/tracing.py's downstream_headers() -- see that
    module's docstring for why (thread/context-safety across generators)."""
    headers = {}
